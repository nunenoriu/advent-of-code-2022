...
class DayFive:
    ...
    def isrykiuok_kortas(self):
        kortos = [8, 9, 5, 3, 6, 13, 2, 1, 7, 10]
        isrykiuotos_kortos = []
        masyvo_ilgis = len(kortos)
        dabartinis_ciklas = 0
        while dabartinis_ciklas < masyvo_ilgis:
            maziausia = None
            i = 0
            j = 0
            while i < len(kortos):
                if maziausia is None or maziausia > kortos[i]:
                    maziausia = kortos[i]
                    j = i
                i += 1
            isrykiuotos_kortos.append(kortos.pop(j))
            dabartinis_ciklas += 1
        print("isrykiuotos_kortos {}".format(isrykiuotos_kortos))
